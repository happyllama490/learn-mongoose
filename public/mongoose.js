...
document.querySelectorAll('#user-list tr').forEach((el) => {
    el.addEventListener('click', function () {
        const id = el.querySelector('td').textContent; // 클릭한 행의 첫 번쨰 셀에 있는 텍스트 콘텐츠(사용자 ID)를 가져옵니다.
...
document.getElementById('user-form').addEventListener('submit', async (e) => {
    ...
    if (!age) {
        return alert('나이을 입력하세요'); // 나이이 입력되지 않은 경우 경고를 표시하고 함수 종료
    }
    try {
...
document.getElementById('comment-form').addEventListener('submit', async (e) => {
    e.preventDefault(); // 기본 동작을 맏ㅅ습니다(페이지 새로고침 방지).
    const id = e.target.userid.value; // 입력된 사용자 ID를 가져옵니다.
    const comment = e.target.comment.value; // 입려된 댓글 내용을 가져옵니다.
    if (!id) {
